...
def apply_conditional_formatting(input_excel_path, output_excel_path, highlight_mode, remove_mode, plant_column_name="Plant", sheet_name=0):
    """
    ...
    Args:
        ...
        plant_column_name (str): The name of the column to check for keywords.
    ...
    if plant_column_name not in df.columns:
        print(f"Error: Column '{plant_column_name}' not found in the sheet.")
    ...
    plant_col_idx = df.columns.get_loc(plant_column_name) + 1 # 1-based index for openpyxl
    remove_index = []
    ...
    for r_idx, row in df.iterrows():
        ...
        cell_value_plant = str(row[plant_column_name]) # Ensure it's a string
...
if __name__ == "__main__":
    ...
    apply_conditional_formatting(input_file, output_file, highlight_mode = False, remove_mode=True, plant_column_name="Unnamed: 2", sheet_name="CleanedData")